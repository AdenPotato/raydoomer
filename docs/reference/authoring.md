...
**Summary (title):** a concise outcome, prefixed by type so the branch is derivable - `Feature: <outcome>` or `Bug: <symptom>` (drives `feature/#<n>` or `bugfix/#<n>`).
...
**Dependencies:** blocked-by / blocks `#<n>`; any cross-repo coordination (a provider API change a consumer needs - see [engine_api_protocol.md](../protocol/engine_api_protocol.md)).
...
**Sizing & priority:** set the GitHub Issues **priority** (`Urgent` / `High` / `Medium` / `Low` / `No priority`) in `/new-work-item`; add an **estimate** if the team sizes work. Split anything that cannot be demoed in one sitting.
...
**Title:** conventional-commit shape + the GitHub issue number - `<type>(<scope>): <description> [#<n>]` (types in [core_protocol.md](../protocol/core_protocol.md#commit-messages---conventional-commits)).
...
**Rules:**
- **One PR per GitHub issue** where practical; the title carries the identifier so GitHub Issues links the PR to the issue.
